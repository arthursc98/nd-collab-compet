...
class MADDPG():
    def __init__(self,
        ...
        self.agents = [Agent(state_size = state_size,
                            ...
                            memory = self.memory,
                            random_seed = random_seed) for i in range(num_agents)]
...
class Agent():
    ...
    def load(self, i):
        self.actor_local.load_state_dict(torch.load(f'checkpoint_actor_{i}.pth'))
        self.critic_local.load_state_dict(torch.load(f'checkpoint_critic_{i}.pth'))

    def save(self, i):
        torch.save(self.actor_local.state_dict(), f'checkpoint_actor_{i}.pth')
        torch.save(self.critic_local.state_dict(), f'checkpoint_critic_{i}.pth')
# endregion [Agents]
